...
void JE_loadItemDat( void )
{
	...
	if (episodeNum >= 4 ) numbspecials = SPECIAL_NUM + 8; /*this ugly hack will need a fix*/

	for (int i = 0; i < numbspecials + 1; ++i)
	{
		fseek(f, 1, SEEK_CUR); /* skip string length */
...
void JE_initEpisode( JE_byte newEpisode )
{
	...
	fprintf(stderr,"****** initializing episode %d ******\n",episodeNum);
	fprintf(stderr,"reading from %s %s %s \n",levelFile,cube_file,episode_file);
	JE_analyzeLevel();
	JE_loadItemDat();
}
